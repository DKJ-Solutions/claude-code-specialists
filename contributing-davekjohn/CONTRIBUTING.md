...
arrangement is why there are exactly four `#` sections below and why every gate sits under the step where it
fires, rather than in a list of its own. Their substeps are `##`, and a gate or a note under one is `###`.
Every level on this page moved one shallower on August 26, 2026, when the four steps became the document's own
top level; the cycle document and `CHANGELOG.md` moved one *deeper* in the same movement, which reads as a
contradiction and is not one — this page is a sequence of four top-level steps, while those two gained a
heading above their contents (`## [Unreleased]`, and a document title that is no longer an H1).
...
## 1A — Create the branch · 1B — Create the empty `development-cycle.md`
...
## 1C — Write `### PLAN` · 1D — `### CREATE` · 1E — `### TEST`
...
must *happen*, DEPLOY says what the change *does*. See 1H for what "secured" means, and why the two are
separate numbers rather than one.

## 1G — Verify every checkbox is resolved
...
one was missed — the step-list gate under 2B is the same question asked by a machine.

## 1H — Wrap and secure DEPLOY: the development cycle is complete
...
explicit *open the PR* command, which is step 2A. That wait is short and usually implicit — see
...
**This is where the waiting ends.** Step 1H leaves the cycle complete and the branch parked on an explicit
...
at this moment: 2B is what it puts in the body, and Gate 3 locks that body against later edits to the
document.

## 2B — Copy the last DEPLOY into the PR
...
## 2D — Copy the last DEPLOY into `CHANGELOG.md` under `## [Unreleased]`, newest to oldest

## 2E — Delete `development-cycle.md`
...
step 2C. Once asked for, the closing steps of that same checklist are covered by the request — including
publishing the GitHub Release. Step 4 below is the one part that is **not**.

**One step of the spec is not here yet, and the letters below therefore differ from
[#894](https://github.com/DaveKJohn/claude-code-specialists/issues/894) by one.** That issue asks for a step
creating *three* kinds of release note under one root —
`releases/changelog/`, `releases/github/` and `releases/audience/` inside this folder — where today
`releases/development/` and `releases/github/` sit at the repo root and only `audience` sits here. That is a
relocation plus a rename, it overlaps the open questions on the release-note layout, and it is a different
subject from the heading levels this page's shift was about, so it stays on #894 for its own branch. When it
lands it inserts as `3B` and everything below it shifts one letter.
...
## 3G — Optional: wait for a `SHIP MAIN` or `PUSH LIVE` command
...
version; a live push changes what customers see. Decision by Dave, August 5, 2026 — restated under 4A, where
it bites.
...
## 4A — Publish and distribute the audience release notes

**The waiting moved up to 3G, and this step is what happens after it** (Dave,
...
section as though the cut had already ended. It has not: 3G is the last thing the cut does, and it is where
