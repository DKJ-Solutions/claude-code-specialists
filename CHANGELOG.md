...
- **Step 3 deleted the document you were reading** (#328). `UNINSTALL.md` is not part of the plugin
  ...
  verification Ã¢â‚¬â€ the last thing removed is the last thing needed Ã¢â‚¬â€ and *Before you start* says to keep a
...
- **Step 4's audit tool went with Step 2** (#328, second half). Step 4 said the teardown's audit *"says
  `[FREE]`"* and offered a re-run *"if you still have the plugin installed"* Ã¢â‚¬â€ which its own Step 2
...
- **#339's open question is answered, and the answer became a rule.** The page used to admit it did not
  ...
  go look. The looking has been done on a virgin profile Ã¢â‚¬â€ the one environment where an earlier install
  could not obscure it: **the clone goes, the unpacked cache stays** (2,930,310 Ã¢â€ â€™ gone; 939,860 Ã¢â€ â€™ still
  there). Stated as the rule rather than the two measurements: **the unpacked cache belongs to the
  marketplace, not to the install** Ã¢â‚¬â€ `marketplace add` creates it (absent Ã¢â€ â€™ 939,768 bytes, install record
...
- **What a torn-down profile actually looks like**, replacing "clean" with something checkable: three files
  exist that were absent before adoption Ã¢â‚¬â€ `installed_plugins.json` (35 bytes), `known_marketplaces.json`
  (288 bytes, no longer naming this marketplace) and `~/.claude/settings.json` (22 bytes) Ã¢â‚¬â€ none holding
...
here Ã¢â‚¬â€ the QUICKSTART, the family README twice Ã¢â‚¬â€ describe the order without numbering it, so none needed a
...
[`UNINSTALL.md`](claude-code-plugins/claude-specialists/UNINSTALL.md) is that page Ã¢â‚¬â€ the mirror of the
