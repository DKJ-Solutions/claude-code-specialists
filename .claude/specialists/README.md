...
> **itself**, via the `dkj-subagents-alpha` plugin (the core team). The team here is therefore small and focused
...
- **`lenses/`** — the **repo layer** of the specialists system: one file per specialist,
  ...
  - **Subagent lens** — for the fifteen specialists that come out of the `dkj-subagents-alpha` plugin as
...
- **Subagent definitions — from the repo's own `dkj-subagents-alpha` plugin, not local.** The compact,
  executable form of a specialist (`<group>-<id>-agent.md`) is **not** kept by this repo in a local
  `.claude/agents/` directory: they come from the `dkj-subagents-alpha` plugin of this very marketplace,
  enabled via [`settings.json`](../settings.json) and invocable as `@dkj-subagents-alpha:<name>`.
- **`settings.json`** — the harness config: `extraKnownMarketplaces` (the `github` source
  ...
  (`dkj-subagents-alpha@claude-code-specialists`). [Sylvester #15](lenses/05-15-extension.md)'s domain.
...
The add-on teams `dkj-subagents-lifehub` and `dkj-subagents-shopify` are **off** here — this repo is
