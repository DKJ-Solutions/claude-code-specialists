...
4. regenerates, for **every** plugin (lockstep — even one untouched this release), its
   **`RELEASE.md`** card (`<plugin>/RELEASE.md`): a `# Release vX.Y.Z` heading, date/type, a
   ...
   [#384](https://github.com/DaveKJohn/davekjohns-workshop/issues/384)). It now hands the "where am I"
   question to the QUICKSTART check that can answer it. Model A (plugin-authored): the card lives
