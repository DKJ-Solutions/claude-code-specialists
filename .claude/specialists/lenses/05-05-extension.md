...
again on an existing branch simply resumes it) and writes `contributing-davekjohn/<branch>.md` in the
...
words are the ones already in the DEPLOY section of `contributing-davekjohn/<branch>.md`. `-Title` is still accepted and ignored, with a
...
description from the changelog entry (the DEPLOY section of `contributing-davekjohn/<branch>.md`), and ticks the two checklist items
...
- **On ONE machine the same parallelism needs a worktree, and the direction is the opposite of the
  ...
  **Shipping from a worktree instead fails harder**, and this was probed rather than reasoned about:
  ...
  [`worktree-lane` skill](../../../plugins/workflows/contributing-davekjohn/skills/worktree-lane/SKILL.md).
...
**mirror**: every script in this section now travels in `contributing-davekjohn`, the opt-in
...
- `scripts/task/new-branch.ps1 -Name <branch-name> [-Title "…"] [-Intent "…"] [-NoPush]` — create (or
  idempotently resume) the branch and, in the same move, write its `contributing-davekjohn/<branch>.md`.
...
- `scripts/task/park-cycle.ps1 [-Quiet]` — **Derek does not run this**, and it is here so he recognises its
  ...
  `contributing-davekjohn/<branch>.md`, and only that file, for the life of the branch — the plan
