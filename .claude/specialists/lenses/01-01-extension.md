...
- **Verify the stand against the repo, not against a handover text.** A session-start briefing — Dave's
  ...
  exists only while a branch is open, so a copy sitting on `main` is a silent half-state — a fold that
  never ran after a merge; since [#1270](https://github.com/DaveKJohn/claude-code-specialists/issues/1270)
  the `unfolded-entry-sessioncheck` hook surfaces this at session start, but read the trunk yourself too:
  the hook reads `origin/<trunk>` without fetching, so a leftover pushed since your last pull is not on it
  yet),
  **`git ls-remote --heads origin` for parked branches** — a parked branch has no PR by design, so every
  ...
  (`check-roster-sync.ps1` + `check-plugin-integrity.ps1` + `check-script-contract.ps1` +
  `check-unfolded-entry.ps1`). Where the
