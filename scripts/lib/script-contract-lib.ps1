...
$script:ContractRecords = @(
    ...
    @{ Lib = 'scripts\repo-config.ps1';     Function = 'Get-MojibakePaths'; Scripts = @('fix-mojibake');
       Adopt = 'copy'; AdoptWhy = 'the one function whose body is workshop-shaped and still safe to copy, because every directory it adds sits behind its own Test-Path: in a repo without plugins/ or releases/ it reduces to the root glob plus contributing-davekjohn/, a superset of the fallback that costs an absent directory nothing. A copy taken before August 14, 2026 still names the retired root branch/ location -- re-adopt to bring the moved folder back under coverage';
    ...
    @{ Lib = 'scripts\repo-config.ps1';     Function = 'Get-ReleaseNotesGrouping'; Scripts = @('cut-release');
       ...
       Returns = "'major' for releases/changelog/<X>.x/ or 'minor' for releases/changelog/<X.Y>/ -- inside whatever root Get-ReleaseDevelopmentNotesRoot answers with, and where the generated notes are foldered, and therefore what the overview row links to. READ IT OFF YOUR EXISTING TREE rather than choosing: a directory named <X>.x (3.x) means major, one named <X.Y> (2.13) means minor, so for any repo that has cut a release before this is a fact to look up, not a decision to make. Answering it differently from the tree you already have starts a second tree beside the first" },
    ...
    @{ Lib = 'scripts\repo-config.ps1';     Function = 'Get-ReleaseHistoryPath'; Scripts = @('cut-release', 'new-internal-note');
       ...
       Optional = $true; Default = 'releases/README.md (source) or contributing-davekjohn/releases/history.md (consumer), computed';
    ...
    @{ Lib = 'scripts\repo-config.ps1';     Function = 'Get-ReleaseGithubNotesRoot'; Scripts = @('cut-release');
       ...
       Optional = $true; Default = 'contributing-davekjohn/releases/github, computed (the same answer for source and consumer since #914)';
    ...
    @{ Lib = 'scripts\repo-config.ps1';     Function = 'Get-ReleaseInternalNotesRoot'; Scripts = @('new-internal-note');
       Adopt = 'copy'; AdoptWhy = 'a location convention, same reasoning and same "no prior seam to redefine" argument as Get-ReleaseDevelopmentNotesRoot above -- read that record first (issue #885, group E). Its own function rather than folded into that one because the two roots are read by different scripts on different days: the cut writes the changelog note at every release, the internal note is a separate, later run';
