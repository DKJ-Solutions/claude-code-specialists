...
.DESCRIPTION
    ...
    the canonical source in plugins/dkj-subagents/subagent-shared/<name>.md and appear
    ...
    'GENERATED, edit subagent-shared/<name>.md' -- a relative path that resolves in THIS repo and nowhere
    else, because plugins/dkj-subagents/subagent-shared/ sits outside every plugin root and therefore does not travel in
    ...
    Shipping subagent-shared/ in the package hands a consumer a file they may open but which is not the
    ...
    is redundant: 'shared:<name>' maps to plugins/dkj-subagents/subagent-shared/<name>.md by construction, which is what
...
function Get-AgentSharedDir {
    ...
    return (Join-Path $RepoRoot 'plugins\dkj-subagents\subagent-shared')
}
...
function Expand-AgentDefShared {
    ...
    while ($i -lt $lines.Count) {
        ...
        if ($null -eq $block) {
            if ($null -ne $Problems) { [void]$Problems.Add("unknown shared block '$name' (source subagent-shared/$name.md missing)") }
