...
.DESCRIPTION
    ...
    THE STEPS, IN ORDER (open a lane):
      ...
      1. Resolve the PRIMARY worktree from git itself (the first entry of `git worktree list
         --porcelain`), never from the caller's cwd -- the whole point is that the caller may be
         standing in a lane.
...
# ---------------------------------------------------------------------------------------------------
# Shared discovery: the primary worktree, from git rather than from cwd.
#
# `git worktree list --porcelain` lists the MAIN worktree first, and that ordering is what makes this
# reliable from inside a lane. Deliberately not `git rev-parse --show-toplevel`, which answers "the
# tree I am standing in" -- the opposite of the question here -- and deliberately not
# CLAUDE_PROJECT_DIR either, which is the harness's project dir and would be the primary only by
# coincidence.
# ---------------------------------------------------------------------------------------------------
$wtRes = Invoke-NativeCapture -FilePath 'git' -Arguments @('worktree', 'list', '--porcelain')
if ($wtRes.ExitCode -ne 0) {
    Write-Error "worktree-lane cannot run -- not a git repository (or git failed to list worktrees)."
    exit 1
}
$worktreePaths = @(
    $wtRes.Output |
...
# Normalise once, here, so every later comparison is between resolved paths rather than between one
# resolved and one as-typed. On Windows the two differ in separator AND in case, and a comparison that
# gets either wrong would silently treat a lane as the primary (or the reverse), which is the one
