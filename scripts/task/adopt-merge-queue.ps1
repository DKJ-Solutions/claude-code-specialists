...
$foldRunner = @(
    ...
    '#   2. the fold REFUSING -- it ran and declined; its own last lines say why. ONE refusal is not in',
    '#      this list and no longer turns the job red at all: the trunk-gap guard (exit 2), which the',
    '#      fold step translates into a stood-down green (#1586, below).',
    ...
    '#',
    '# SO THIS JOB STANDS DOWN ON THAT ONE REFUSAL rather than going red -- exit code 2 from the fold,',
    ...
    '# before a single entry is folded. Every OTHER non-zero code still fails the job.',
    '#',
