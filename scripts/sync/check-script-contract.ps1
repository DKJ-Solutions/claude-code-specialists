...
# contributing-davekjohn/ is where everything portable about the workflow gathers in a consumer: the folder
...
# 'contributing-davekjohn/', and this line is forwarded by a SessionStart hook as an [ERROR]. Checking only
# the new name would greet every unmigrated consumer with "your folder does not exist" about a folder that
# is sitting right there -- the loudest possible way to be wrong. The two names are literals rather than a
# read of Get-BranchFilePaths because this script dot-sources only check-report-lib and script-contract-lib;
# adding a third dependency to answer "does a directory exist" costs more than it buys. If a fourth name
# ever appears, that trade flips.
$workflowFolderNames = @('contributing-davekjohn', 'workflow-davekjohn')
...
if (-not $foundWorkflowFolder) {
    Write-Failure ("the workflow folder 'contributing-davekjohn/' does not exist in this repo -- since " +
