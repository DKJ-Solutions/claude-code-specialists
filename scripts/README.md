...
| [`task/plugin-versions.ps1`](task/plugin-versions.ps1) | per enabled plugin: the version installed in **this** checkout vs. the marketplace clone's `plugin.json` version + git HEAD, with a verdict on whether a plugin update is due — read-only, runs on any device | `plugin-versions` |
