...
'@
    ...
    Write-Host ''
    Write-Host 'Get-SyncMergedRefTips / Test-SyncRefMergedByPr -- was THIS ref merged, or just its name?' -ForegroundColor Cyan

    # WHAT THESE CASES PROTECT is a false negative that arrived through the guard's own answer (inbound
    # #1190). These branch names are date-stamped, so a name whose branch merged and was DELETED comes
    # round again -- and matching the bare name let the merged branch vouch for the brand-new, unmerged
    # one standing in its place. The run then reported 'all merged', found nothing standing, and pushed a
    # '-2' branch onto the pile the guard exists to prevent.
    #
    # NOTHING HERE REACHES gh, and it does not have to: the parse and the decision are the two halves that
    # were wrong, and both are pure. The suite for the script proper cannot drive this at all -- its
    # fixture's origin is a local bare repo, so 'gh pr list' has no repository to answer for -- which is
    # exactly why the decision was worth lifting out of the loop and into here.
    $tipMerged   = ('a' * 39) + '1'
    $tipStanding = ('b' * 39) + '2'
    $tipOther    = ('c' * 39) + '3'
    $reused      = 'sync/live-2026-09-01'

    $tips = Get-SyncMergedRefTips -Lines @(
        "$reused`t$tipMerged",
    ...
    Assert-Equal 2 $tips.Count 'tips: only the rows shaped "<name> TAB <sha>" are read'
    Assert-True ($tips.ContainsKey($reused)) 'tips: and the merged branch is one of them'
    Assert-True (-not $tips.ContainsKey('sync/live-2026-08-29')) 'tips: a row whose headRefOid was null is dropped rather than stored as a tipless match'
    Assert-True (-not $tips.ContainsKey('sync/live-2026-08-28')) 'tips: a second field that is not an object name is not a tip'

    # THE DEFECT ITSELF, in one assert: same name, different commit. The merged PR is real, and it is not
    # about this ref.
    Assert-True (-not (Test-SyncRefMergedByPr -Name $reused -Tip $tipStanding -MergedTips $tips)) `
    ...
    Assert-True (Test-SyncRefMergedByPr -Name $reused -Tip $tipMerged -MergedTips $tips) `
    ...
    # answer: it holds a commit the trunk has not seen, so it is a predecessor.
    Assert-True (-not (Test-SyncRefMergedByPr -Name 'sync/live-2026-08-30' -Tip $tipStanding -MergedTips $tips)) `
        'merged/moved: a ref that has moved on since its PR merged is standing again'

    Assert-True (Test-SyncRefMergedByPr -Name $reused -Tip $tipMerged.ToUpperInvariant() -MergedTips $tips) `
        'merged/sha-case: an object name is hex, so the comparison does not care which case it arrives in'

    # THE THREE UNKNOWNS, all answering 'standing'. That is the caller's own direction rather than a rule
    # of its own: a run that cannot be proven safe is refused, loudly, naming its override.
    Assert-True (-not (Test-SyncRefMergedByPr -Name $reused -Tip '' -MergedTips $tips)) `
        'merged/no-tip: an unreadable tip is not merged'
    Assert-True (-not (Test-SyncRefMergedByPr -Name $reused -Tip $tipMerged -MergedTips $null)) `
        'merged/gh-silent: a gh that could not answer leaves every branch standing'
    Assert-True (-not (Test-SyncRefMergedByPr -Name 'sync/live-2026-07-01' -Tip $tipMerged -MergedTips $tips)) `
        'merged/absent: a name no merged PR carried is standing, even at a tip some other PR did merge'

    # ORDINAL ON THE NAME. A hashtable's default comparer is case-insensitive, which would let a merged
    # 'Sync/...' vouch for a standing 'sync/...'; git refs are case-sensitive, so those are two branches.
    Assert-True (-not (Test-SyncRefMergedByPr -Name $reused.ToUpperInvariant() -Tip $tipMerged -MergedTips $tips)) `
        'merged/name-case: a name differing only in case is a different branch'

    # ONE NAME, TWO MERGED PRS -- the ordinary end state of the reused-name case above, once the second
    # one lands too. Each tip vouches for itself and neither vouches for a third.
    $twice = Get-SyncMergedRefTips -Lines @("$reused`t$tipMerged", "$reused`t$tipOther", "$reused`t$tipMerged")
    Assert-Equal 1 $twice.Count 'tips/twice: two merged PRs on one name are one entry'
    Assert-True (Test-SyncRefMergedByPr -Name $reused -Tip $tipMerged -MergedTips $twice) 'tips/twice: the first tip still proves itself'
    Assert-True (Test-SyncRefMergedByPr -Name $reused -Tip $tipOther  -MergedTips $twice) 'tips/twice: and so does the second'
    Assert-True (-not (Test-SyncRefMergedByPr -Name $reused -Tip $tipStanding -MergedTips $twice)) 'tips/twice: a third tip on that name is still standing'

    Assert-Equal 0 (Get-SyncMergedRefTips -Lines @()).Count 'tips: no merged PRs is an empty lookup rather than a throw'
    Assert-Equal 0 (Get-SyncMergedRefTips).Count 'tips: and so is no argument at all'
}
finally {
