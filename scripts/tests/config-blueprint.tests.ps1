...
$Artefact   = Join-Path $RepoRoot 'plugins\specialists-workflow-davekjohn\blueprint\config-blueprint.json'
...
# The source leaves two contract functions at the built-in fallback. Recorded as declared=false with no
# text rather than dropped, because "this repo does not state it either" is an answer.
$undeclared = @($bp.records | Where-Object { -not $_.declared })
Assert-Equal 2 $undeclared.Count 'the two functions the source itself leaves at the fallback are recorded, not dropped'
