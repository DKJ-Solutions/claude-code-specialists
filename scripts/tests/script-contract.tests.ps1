...
try {
    ...
    Assert-Match 'Summary: 0 error\(s\), 11 info signal\(s\)' $r.Out 'happy path: [SCOPE] is non-counting (0 errors; the five deliberately-undefined seams -- the two impact-table ones, Get-TestCommands since inbound #644, Get-EntryGateExemptPrefixes since inbound #789, and Get-ReleasePageMasthead since inbound #809 -- plus two reachability signals: neither fold-changelog-entry nor new-internal-note can see this consumer''s Get-BranchTypes, which is the ordinary state for a repo that has not chained branch-info.ps1 into its repo-config -- plus the four optional seams issue #885 added (Get-ChangelogPath, Get-ReleaseChangelogNotesRoot, Get-ReleaseGithubNotesRoot, Get-ReleaseInternalNotesRoot), each with a computed default this fixture does not define)'
    ...
    Assert-Equal 15 $infoCount6e 'stub wording absent: exactly fifteen [INFO] lines -- one per stripped knob, plus the five seams this repo deliberately never defines (the two impact-table ones, Get-TestCommands since inbound #644, Get-EntryGateExemptPrefixes since inbound #789 -- this repo runs no mirror branches, so its default IS its answer -- and Get-ReleasePageMasthead since inbound #809, this repo having no wordmark), the two reachability signals on Get-BranchTypes, and the four optional seams issue #885 added (Get-ChangelogPath, Get-ReleaseChangelogNotesRoot, Get-ReleaseGithubNotesRoot, Get-ReleaseInternalNotesRoot), and nothing else downgraded along with them. Was eight until the flat changelog retired the superseded Get-ChangelogHeading and Get-ChangelogReleaseWording records, then six until inbound #580 added a record whose seam a consumer leaves unreachable, then eight until #644, then nine until #789, then ten until #809, then eleven until #885'
