...
### The release overview leaves the README and becomes the file the seam already points at
...
### The changelog keeps the latest release, not all seventy-two
...
### The last undocumented mirror gets its page, and check 18's coverage line is down to one honest exemption
...
### The safety-critical ship-pr sequence gets the page it was already being sent to
...
### A shared script's parameters must appear in the skill that documents it
...
### The latest-release block loses its redundant heading and stops accumulating blank lines
...
### The only two required contract entries a consumer cannot decline become optional
...
### The roster check inspects the version a session actually loads, not the highest one in the cache
...
### The em-dash defect was in the test fixture, and the weakened assert was hiding it
...
### The internal note's open section is a snapshot, because it is published
...
### Two release attachments cannot share a filename, and the checklist did not say so
...
#### Name the mechanism behind the broken fixture, and its stiller second form
...
#### Move the git and gh craft from Derek's lens into the shared source
...
#### The release craft moves from the lens into the shared source
...
#### In the source repo the source is the default destination, not the lens
...
#### The written notes for v3.3.0: the internal summary and the highlights
