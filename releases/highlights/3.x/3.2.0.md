...
### Turn the highlights tier on: this repo cuts a stakeholder document too
...
### The highlights tier moves to the shared cut
...
### cut-release moves to the shared mirror, with the repo differences in the seam
...
### Make a dead persona-body import loud instead of silent
...
### Mirror ship-pr, verify-resolved-issues and fix-mojibake into the plugin
...
### Repo-configurable stub wording for a changelog entry
...
### Flatten the plugin directory layer into plugins/ and give the repo one landing page
...
### a round's step-table totals are counted, not typed
...
### A round's baseline table is measured, not typed
...
### The teardown audit walks every file under the scanned roots, and now says so
...
### Repair the stale marketplace name in generated intros
...
### Wrap-proof the captured-output asserts in the branch test suites
...
### the release card states what it describes, not where you are
...
### the untouched-install note is gated on the install record
...
### the #327 disarm claim is conditional on which key stays behind
...
<!-- Remove this block before sharing the highlights with non-developers. -->
## For developers only -- remove before publishing

### Documentation

#### QUICKSTART becomes ADOPTION.md, with a real quickstart beside it
...
#### A folder rename silently unlinks plugin installs
...
#### a reported finding's reason is verified before it is repaired
...
#### two promises the consumer path cannot keep are made conditional
...
#### the honest leftovers list gains a row and loses a stale citation
...
#### rename the marketplace to claude-code-specialists
