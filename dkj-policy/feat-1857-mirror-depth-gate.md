...
A shared script exists twice at different depths; the drift lint proves the two copies identical as TEXT, and nothing proves identical BEHAVIOUR where a path resolves off $PSScriptRoot across the plugin-root boundary. Measured: 2 of 34 entry points do that, and neither has a suite that runs its mirror.
...
nothing above its own `scripts\` folder -- so it proves a dot-source, not a depth. The two scripts that
...
the file in both copies; flagging it would bury the two crossings under thirty-two that cannot differ.

Both crossings were already correct. What was missing was the proof: they had been verified by hand,
and the copy that fires in every released install is the one no suite executed. Both now do.
