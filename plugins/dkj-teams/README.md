...
|---|---|
| [`dkj-team-alpha/`](dkj-team-alpha/) | The **core team** — the repo-neutral specialists who work the same way in every repo, plus the persona templates of the main loop and the bootstrap skill `specialists-init`. |
| [`dkj-team-lifehub/`](dkj-team-lifehub/) | An **add-on team** for a personal information hub / brain-based knowledge repo. |
| [`dkj-team-shopify/`](dkj-team-shopify/) | An **add-on team** for a Shopify store repo: theme code, store management, configuration. |
| [`dkj-team-ecomm/`](dkj-team-ecomm/) | An **add-on team** for a commercial webshop of any platform: SEO, CRO, performance/SEA. |
...
`dkj-team-alpha` is the foundation and belongs in every consuming repo. Each further team **adds
specialists**, so enabling two of them raises no conflict — a Shopify store repo that is also a
commercial webshop legitimately enables `dkj-team-shopify` and `dkj-team-ecomm` alongside the core. That is
...
A team is named `team-<name>` and lives under `plugins/dkj-teams/`. Since August 9, 2026 that pairing is
...
- **`skills/`, `hooks/`, `scripts/`, `personas/`** — optional, and only where a team genuinely needs
  them. In practice `dkj-team-alpha` is the one carrying all four, because the adoption path, the roster
  and workflow session checks and the sync scripts belong to the core rather than to any domain team;
  `dkj-team-shopify` ships four domain skills, and the other two teams are specialists and manuals only.
