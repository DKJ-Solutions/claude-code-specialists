...
description: Place dkj-team-shopify's operational floor in this repo, in one move -- the live-theme guard's seam (which theme is live), a starter theme-check config that is green on arrival in a real theme, and the CI workflow that runs it on every PR. Use this right after enabling or refreshing dkj-team-shopify, since a plugin install writes nothing into a repo, and whenever the Shopify floor session check reports that the guard has not been told which theme is live. Strictly additive and dry-run by default; it never overwrites anything.
---

# adopt-shopify-floor -- the floor arrives with the plugin, the answers do not

`dkj-team-shopify` ships a `PreToolUse` guard on the live theme, and it starts working the moment the plugin
...
[the dkj-team-shopify README](../../README.md#converging-off-a-hand-written-guard). Removing a `PreToolUse`
