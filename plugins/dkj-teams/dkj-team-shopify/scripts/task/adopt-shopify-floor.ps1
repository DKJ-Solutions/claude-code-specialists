<#
.SYNOPSIS
    Adopts dkj-team-shopify's operational floor into a consuming Shopify repo: the guard's two seams, a
    starter theme-check config, and the CI workflow that runs it (issues #769, #776).

.DESCRIPTION
    THE PLUGIN SHIPS THE FLOOR; NOTHING SHIPPED THE INSTALL PATH. dkj-team-shopify's PreToolUse guard
    ...
    dkj-team-shopify's own README warns about. adopt-config settled the same question the same way for its
...
function Get-SeamBlock([string]$Id, [string]$StoreDomain) {
    ...
    $lines = @(
        '',
        '# --- dkj-team-shopify: the live-theme guard''s two seams ----------------------------------------------',
        '#',
        '# Placed by adopt-shopify-floor. The guard is a PreToolUse hook in the dkj-team-shopify plugin: it',
    ...
    $lines += @(
        ...
        '',
        '# --- dkj-team-shopify: the pre-task sync ------------------------------------------------------------',
        '#',
...
# for local runs. Placed by dkj-team-shopify's adopt-shopify-floor as a STARTER: it is yours now, and
...
#
# Placed by dkj-team-shopify's adopt-shopify-floor. It is this repo's workflow now.
...
switch ($seamState) {
    ...
    'no-lib'  { Write-Host "  [STOP]   $configRel does not exist -- type /dkj-team-alpha:specialists-init first (reserved for explicit user invocation, so an agent hands it to the repo owner); it owns that file's existence." -ForegroundColor Yellow }
    default   {
...
Write-Host 'measurements are in the dkj-team-shopify README, under the git status section.'
Write-Host ''
Write-Host 'Already had a hand-written guard? See "Converging off a hand-written guard" in the'
Write-Host 'dkj-team-shopify README before you keep both -- two PreToolUse hooks fire on every command.'
