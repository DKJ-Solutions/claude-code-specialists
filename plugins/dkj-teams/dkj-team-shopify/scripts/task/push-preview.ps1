...
.DESCRIPTION
    ...
    outright here, and dkj-team-shopify's PreToolUse guard blocks a push aimed at live independently of this
    script.
...
# load rather than push a theme whose failure path cannot be reached. Registered for dkj-team-shopify in
...
if ($liveId -notmatch '^\d+$') {
    ...
    Write-Warning ("Get-ShopifyLiveThemeId does not answer with a theme id, so this script cannot " +
        "recognise the live theme by id. dkj-team-shopify's guard hook still blocks a live-aimed push; answer " +
