...
.DESCRIPTION
    ...
    lint and test gates along with it: that would couple dkj-team-shopify to dkj-policy, and the
    ...
    writes to git. dkj-team-shopify's PreToolUse guard covers those three acts independently and is not
    weakened here.
...
# without it must fail at load rather than silently push unbounded. It travels in dkj-team-shopify's own
...
# rather than run a pull whose failure path cannot be reached. It travels in dkj-team-shopify's own payload,
...
# --- The seam answers ------------------------------------------------------------------------------
# Read in a child scope with StrictMode OFF and inside a try, exactly as dkj-team-shopify's live-theme guard
