...
try {
    ...
    if ($errorCount -gt 0) {
        ...
        Write-Host '  (run /dkj-team-alpha:sync-roster to stage the catch-up -- that command must be TYPED'
        ...
        if ($orphanLines.Count -gt 0) {
            ...
            Write-Host '  (run /dkj-team-alpha:sync-roster to stage the catch-up -- that command must be'
