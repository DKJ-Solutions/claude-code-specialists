...
description: >-
  ...
  Use this when the shared `dkj-team-alpha` plugin is installed and enabled but the conductor and the
...
The shared `dkj-team-alpha` plugin delivers the **worker subagents** (Sylvester, Tessa, Edith, Victor,
...
> [claude-code-specialists README](https://github.com/DaveKJohn/claude-code-specialists/blob/main/)
...
This skill lives inside the `dkj-team-alpha` plugin, so it only becomes available once the plugin is
...
`/dkj-team-alpha:specialists-init`, and there is **no per-repo escape**: `skillOverrides` only ever
...
`/dkj-team-alpha:specialists-init` in full and then says the repo owner has to type it.
...
> (That third page is [`INSTALL.md`](https://github.com/DaveKJohn/claude-code-specialists/blob/main/) today: the
...
>
> **That adoption page is [`plugins/ADOPTION.md`](https://github.com/DaveKJohn/claude-code-specialists/blob/main/).** It has been three
...
> [`INSTALL.md`](https://github.com/DaveKJohn/claude-code-specialists/blob/main/) quickstart counts
...
"enabledPlugins": {
  ...
  // plus a domain plugin of choice, e.g. "dkj-team-shopify@claude-code-specialists": true
}
```
...
```
✘ Failed to update plugin "dkj-team-alpha@claude-code-specialists": Plugin "specialists" is not installed at scope user
```
...
[adoption page](https://github.com/DaveKJohn/claude-code-specialists/blob/main/), and in the release notes. Read a bare
...
```
❯ dkj-team-alpha@claude-code-specialists   Version: 3.0.1   Scope: project   Status: ✔ enabled
```
...
on `main` — see [Staying up to date](https://github.com/DaveKJohn/claude-code-specialists/blob/main/) for why that happens
...
described in the [claude-code-specialists README](https://github.com/DaveKJohn/claude-code-specialists/blob/main/). Every reader accepts both
