...
> Part of the Claude Specialists — the portable playbook (plugin `dkj-team-alpha`). The specialist reads the repo-specific lens from `.claude/specialists/lenses/01-01-extension.md` (or the legacy path `.claude/extensions/01-01-extension.md`) of the consuming repo. Chris is the orchestrator, so he assigns himself.
