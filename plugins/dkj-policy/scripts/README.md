...
| `lib/closeout-lib.ps1` | `Write-CloseOutReceipt` — the close-out receipt shape, printed as the last line of the five scripts that end a work chain: what happened, where to read it, whether the session can be cleared, and the clause a run that skipped a gate owes the PR body. The one thing here addressed to the READER of a run rather than to the run | none — dot-sourced lib |
