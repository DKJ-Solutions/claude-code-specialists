...
| `maintenance/tidy-machine.ps1` | the whole-machine tidy in ten lanes — stale worktree lanes, branches whose PR was CLOSED without merging, expired `backup/*` branches, old stashes, orphaned install records, fixture trees under the scratch root. A conductor: six lanes call a script already in this table, and it deletes only through `prune-merged`'s own two proofs | [`tidy-machine`](../skills/tidy-machine/SKILL.md) |
