...
| [`tidy-machine`](skills/tidy-machine/SKILL.md) | the wider clutter, in one command and ten lanes — stale worktree lanes, branches whose PR was **closed** without merging, expired backups, old stashes, orphaned install records, fixture trees under your temp directory. It deletes only what `prune-merged` can already prove and hands the rest over with the command |
