...
under [`dkj-subagents/`](dkj-subagents/) — or *how does work move through this repo* — then it is a **workflow**, and the
...
`dkj-subagents/` holds the teams *of this family*, and `dkj-policy/` names the **government** — the prime
...
[`[plugin-kind]` check](../scripts/lint/check-plugin-integrity.ps1) holds `dkj-subagents-*` and the policy
...
| **Ships** | `subagents/` + `manuals/` — one subagent definition and one portable playbook per specialist | `skills/`, and the scripts and hooks behind them — no specialists at all |
...
| **Named** | `dkj-subagents-<name>`, under `plugins/dkj-subagents/`; bare `team-*` and the retired `dkj-team-*` are still accepted names but sit nowhere in particular | `<owner>-policy` and `<owner>-policy-<ministry>`, under `plugins/dkj-policy/`; `workflow-*`, `contributing-*` and `*-codex` are still accepted names but sit nowhere in particular |
...
- **A plugin's name says which kind it is, and where a name claims a directory it must sit there.** Two
  shapes claim one: `dkj-subagents-*` claims `plugins/dkj-subagents/`, and `*-policy` / `*-policy-*` claim
...
Both are stated in full where they apply: [`dkj-subagents/README.md`](dkj-subagents/README.md) and
...
[`dkj-subagents/subagent-shared/`](dkj-subagents/subagent-shared/) — a generator writes those blocks *into* the team folders,
and it has [its own README](dkj-subagents/subagent-shared/README.md) for the
never-edit-between-the-sentinels rule and how to add a block. It sits inside `dkj-subagents/` because every file
