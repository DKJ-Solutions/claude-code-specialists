...
# scripts/task/ and the artefact is under plugins/. Tried in that order: a consumer must find the
# plugin's copy, never a stray one.
function Resolve-Blueprint {
    ...
    # All three are $PSScriptRoot-relative first and $RepoRoot-relative last, deliberately: the artefact
    # travels with the SCRIPT, and a consumer whose CLAUDE_PROJECT_DIR points somewhere else entirely
    # must still find the blueprint next to the file it is running.
    $candidates = @(
        # the plugin mirror: <plugin>/scripts/task/ -> <plugin>/blueprint/
        (Join-Path $PSScriptRoot '..\..\blueprint\config-blueprint.json'),
        # the workshop root copy: scripts/task/ -> plugins/<plugin>/blueprint/
        (Join-Path $PSScriptRoot '..\..\plugins\specialists-workflow-davekjohn\blueprint\config-blueprint.json'),
        # last resort, for a checkout laid out differently from either
        (Join-Path $RepoRoot 'plugins\specialists-workflow-davekjohn\blueprint\config-blueprint.json')
    )
    foreach ($c in $candidates) {
        if (Test-Path -LiteralPath $c -PathType Leaf) { return (Resolve-Path -LiteralPath $c).Path }
    }
    throw "the config blueprint could not be found. It ships with the specialists-workflow-davekjohn plugin at blueprint/config-blueprint.json; if that plugin is enabled, update it (claude plugin update specialists-workflow-davekjohn@claude-code-specialists --scope project)."
}
