...
daily reconciliation sweep repairs anything a missed event left behind. **A ticket that came the
other way -- filed in Asana and copied into an issue for analysis -- is covered too:** the workflow
reads the Asana link in such an issue's header row when it carries no machine marker of its own.
