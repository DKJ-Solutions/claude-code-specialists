...
`CLAUDE.md`, on the grounds that an always-on document already stated the same three rules and a second
copy is a thing to keep in sync rather than a safety net. **Nothing in this workflow depends on that
choice** — every gate reads your branch's own `development-cycle.md`, never a contributing page — so
both answers work.
...
repository sidebar, and it only recognises that file in the root, `.github/` or `docs/` — a page in
