...
function Resolve-Blueprint {
    ...
    throw "the config blueprint could not be found. It ships with the workflow plugin at blueprint/config-blueprint.json; if that plugin is enabled, update it (claude plugin update contributing-davekjohn@claude-code-specialists --scope project)."
}
...
if ($missingLibs.Count -gt 0) {
    ...
    # NAMES THE COMMAND AND WHO TYPES IT (inbound #1093 / #1096). Not a SessionStart hook like the other
    # two sites, but the same trap: a model that reached this [STOP] is told to run a skill the harness
    # will refuse it. Reasoning in full at check-roster-sync.ps1's [BOOTSTRAP] marker.
