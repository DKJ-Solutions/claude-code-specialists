...
$script:ContractRecords = @(
    ...
    @{ Lib = 'scripts\repo-config.ps1';     Function = 'Get-ChangelogPath'; Scripts = @('cut-release', 'fold-changelog-entry', 'new-branch', 'open-pr');
       Adopt = 'copy'; AdoptWhy = "a location convention. The computed default is 'CHANGELOG.md' when .claude-plugin/marketplace.json exists (the workflow's SOURCE publishes plugins) and 'contributing-davekjohn/CHANGELOG.md' otherwise -- the same one-file test Get-ReleasePluginTier's fallback already uses, applied here so a consumer that configures nothing is isolated by default (issue #885). AMENDED, AUGUST 27, 2026 -- amended rather than silently flipped, as every relocated seam's record here is. This paragraph used to end by saying the source's own declaration 'would only restate what the computation already answers correctly', and that was true for as long as the source kept its changelog at its root. Dave moved it into contributing-davekjohn/ together with the contributing page and the release history, so the source now DIFFERS from its own computed answer and states the seam to say so -- which is what the seam is for. NOTHING CHANGES FOR A CONSUMER: the default is unchanged, and it has pointed into the workflow folder since #885. What changed is that the source stopped being the one repo answering it the other way";
