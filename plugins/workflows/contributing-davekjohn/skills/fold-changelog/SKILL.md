...
description: >-
  ...
  (the DEPLOY section of contributing-davekjohn/development-cycle.md, or an older branch/ pair, or a pre-split
...
**The entry is the DEPLOY section of `contributing-davekjohn/development-cycle.md`** — a fixed path, the same on
...
Without `-Branch` it folds everything it finds: `contributing-davekjohn/development-cycle.md` if it holds an entry, plus
...
1. Folds each entry into `CHANGELOG.md`, with the PR number + link included (retrieved via
   ...
   on its file name).
2. Clears it afterwards: **`contributing-davekjohn/development-cycle.md` is removed** — one deletion, which
