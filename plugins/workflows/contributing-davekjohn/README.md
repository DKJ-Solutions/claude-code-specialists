...
| [`hooks/`](hooks/) | three read-only SessionStart checks that never block, all belonging to running this across several repos: `connector-sessioncheck`, `script-contract-sessioncheck` and `unfolded-entry-sessioncheck` (the trunk carries a branch's `development-<branch>.md` that a fold should have removed — #1270) — plus one **Stop** hook that acts rather than reports: `cycle-autopark` pushes the branch's `development-<branch>.md` to `origin` after every turn, until a PR publishes it (#900) |
