...
$script:ContractRecords = @(
    ...
    @{ Lib = 'scripts\repo-config.ps1';     Function = 'Get-ReleaseHistoryPath'; Scripts = @('cut-release', 'new-internal-note');
       Adopt = 'copy'; AdoptWhy = "a location convention rather than a fact about the repo. REVERSED AGAIN, ISSUE #885, AUGUST 25, 2026 -- amended rather than silently flipped, per Dave's own instruction on that issue. The August 19, 2026 answer (moved back to releases/README.md) rested on one premise: `a folder a teardown removes` should not hold a repo's only list of releases. That premise no longer holds -- #885 also settled that workflow-davekjohn/ is PERMANENT, no command in this plugin removes it and no future teardown may -- so the durability worry that sent the list back to the root is answered a different way, and the folder becomes the safer place for it rather than the riskier one. The computed default now isolates a consumer the same way Get-ChangelogPath does: source keeps releases/README.md (unchanged for THIS repo -- copying it here states what is already true), a consumer gets workflow-davekjohn/releases/history.md (a NEW filename, not README.md, because that name is already this folder's seam-ANSWERS page). ACCEPTED COST, same shape as the changelog: an existing consumer's history splits at the point this default starts applying -- old rows stay at their root file, new rows land in the folder -- rather than moving under them a second time without their say. A consumer who would rather keep one list repoints the seam back to their existing root file";
       Optional = $true; Default = 'releases/README.md (source) or workflow-davekjohn/releases/history.md (consumer), computed';
    ...
    @{ Lib = 'scripts\repo-config.ps1';     Function = 'Get-ReleaseDevelopmentNotesRoot'; Scripts = @('cut-release', 'new-internal-note');
       Adopt = 'copy'; AdoptWhy = "a location convention, computed the same way as the seams above it (issue #885, group E). UNTIL THIS BRANCH THIS ROOT CARRIED NO SEAM AT ALL, by deliberate refusal recorded at its old hard-coded assignment: `a seam nobody can be shown to need is a knob a consumer has to read past. It comes back when somebody measures it` -- #885 is that measurement. Because there was no prior seam, there is no prior meaning a computed default could silently redefine: every existing consumer's development notes already sit at the literal 'releases/development' this default still returns for the source, and a consumer newly isolating gets workflow-davekjohn/releases/development instead";
       Optional = $true; Default = 'releases/development (source) or workflow-davekjohn/releases/development (consumer), computed';
       Returns = "the repo-root-relative directory the cut writes the generated development note (tier 0, always written, complete and long) into, grouped by Get-ReleaseNotesGrouping underneath it. Read by cut-release.ps1 (writes it every release) and new-internal-note.ps1 (reads the development note back in as input for the internal note it drafts, so the two must agree)" },
    @{ Lib = 'scripts\repo-config.ps1';     Function = 'Get-ReleaseGithubNotesRoot'; Scripts = @('cut-release');
       Adopt = 'copy'; AdoptWhy = 'a location convention, same reasoning and same "no prior seam to redefine" argument as Get-ReleaseDevelopmentNotesRoot above -- read that record first (issue #885, group E)';
       Optional = $true; Default = 'releases/github (source) or workflow-davekjohn/releases/github (consumer), computed';
       Returns = 'the repo-root-relative directory the cut writes the generated GitHub Release body into. The one generated document that IS published, which is why it has its own root rather than sharing releases/development/ (Dave, August 12, 2026) -- the directory whose whole job is the record nobody publishes' },
    @{ Lib = 'scripts\repo-config.ps1';     Function = 'Get-ReleaseInternalNotesRoot'; Scripts = @('new-internal-note');
       Adopt = 'copy'; AdoptWhy = 'a location convention, same reasoning and same "no prior seam to redefine" argument as Get-ReleaseDevelopmentNotesRoot above -- read that record first (issue #885, group E). Its own function rather than folded into that one because the two roots are read by different scripts on different days: the cut writes the development note at every release, the internal note is a separate, later run';
       Optional = $true; Default = 'releases/internal (source) or workflow-davekjohn/releases/internal (consumer), computed';
       Returns = "the repo-root-relative directory new-internal-note.ps1 writes the tier-1 internal note's skeleton into. Must agree with cut-release.ps1's own read of Get-ReleaseDevelopmentNotesRoot's tier -- both roots hold documents from the SAME release, at the SAME grouped sub-path" },
    @{ Lib = 'scripts\repo-config.ps1';     Function = 'Get-ChangelogPath'; Scripts = @('cut-release', 'fold-changelog-entry', 'session-status');
       Adopt = 'copy'; AdoptWhy = "a location convention, and its computed default already states this repo's own answer without being copied: 'CHANGELOG.md' when .claude-plugin/marketplace.json exists (this repo publishes plugins), 'workflow-davekjohn/CHANGELOG.md' otherwise -- the same one-file test Get-ReleasePluginTier's fallback already uses, applied here so a consumer that configures nothing is isolated from this repo's own root changelog by default, and this repo's own explicit declaration would only restate what the computation already answers correctly (issue #885)";
       Optional = $true; Default = 'CHANGELOG.md (source) or workflow-davekjohn/CHANGELOG.md (consumer), computed';
