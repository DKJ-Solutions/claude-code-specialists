...
$script:ContractRecords = @(
    ...
    @{ Lib = 'scripts\repo-config.ps1';     Function = 'Get-ReleaseHistoryPath'; Scripts = @('cut-release', 'new-internal-note');
       Adopt = 'copy'; AdoptWhy = 'a location convention rather than a fact about the repo, and since August 19, 2026 the convention IS the default (releases/README.md) -- so copying it states what would already have been true. It pointed into the workflow folder between August 14 and 19; that was wrong for a reason worth keeping: a repo that has cut releases has a HISTORY whichever tooling cut it, so an index of files in releases/ does not belong in a folder a teardown removes. Get-ReleaseNoteRoot is the opposite case and still points in there, because a per-reader note exists only BECAUSE the tier model does. What does NOT travel with the copy is the precondition: since the changelog stopped carrying release blocks this file is the only list of releases there is, so the consumer has to keep it complete';
